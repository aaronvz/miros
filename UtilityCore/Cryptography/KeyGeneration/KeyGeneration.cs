...
namespace UtilityCore.Cryptography
{
    ...
    {
        public static void GenerateKey(string username, string password, string keyStoreUrl, string nombreLlavePublica, string nombrellavePrivada)
        {
            ...
            FileStream out1 = new FileInfo(string.Format("{0}" + nombrellavePrivada, keyStoreUrl)).OpenWrite();
            FileStream out2 = new FileInfo(string.Format("{0}" + nombreLlavePublica, keyStoreUrl)).OpenWrite();
        ...
        public static void ExportKeyPair(Stream secretOut, Stream publicOut, AsymmetricKeyParameter publicKey, AsymmetricKeyParameter privateKey, string identity, char[] passPhrase, bool armor)
        {
            if (armor)
            {
